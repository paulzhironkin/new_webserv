...
int Request::store(char buffer[REQ_BUF_SIZE], struct pollfd *ptr_tab_poll, int bytes_to_read) {
...
void Request::parseText(struct pollfd *ptr_tab_poll, int port)
{
...
void		Request::composeReponse(struct pollfd *ptr_tab_poll)
{
...
int	Request::sendResponse(struct pollfd *ptr_tab_poll) {
	...
	size_t i;
	for (i = 0; i < rsize && i < REQ_BUF_SIZE; i++) {
	...
	ret = send(fd, buf, (REQ_BUF_SIZE < rsize ? REQ_BUF_SIZE : rsize), 0);
...
bool	Request::isFullyReceived(struct pollfd *ptr_tab_poll) {
    ...
    else if ((pos = ci_find_substr(request_str, "content-length")) != -1) 
    {
        ...
        rqcp.erase(0, pos + 15);
        if (rqcp[0] == 13 || rqcp[0] == 32)// \r or space smbols
            rqcp.erase(0, 1);
        int str_len = 0;
...
std::string Request::get_path()				{return(header["url"]);}
std::string Request::get_body()				{return(header["body"]);}
std::string Request::get_host()				{return(header["HOST"]);}
std::string Request::get_method()			{return(header["method"]);}
std::string Request::get_accept()			{return (header["ACCEPT"]);}
std::string Request::get_content_type()		{return(header["CONTENT-TYPE"]);}
std::string Request::get_content_length()	{return(header["CONTENT-LENGTH"]);}
std::string Request::get_accept_encoding()	{return(header["ACCEPT-ENCODING"]);}
std::string Request::get_accept_language()	{return(header["ACCEPT-LANGUAGE"]);}
